...
using components = std::tuple<display::Colour, double, double, double, double>;
...
class Object {
public:
  Object(std::shared_ptr<Texture_Material> txt) : txt_{txt} {
...
public:
  Sphere(const structures::Vec3 &center, std::shared_ptr<Texture_Material> txt,
         const double &radius)
      : Object(txt), r_{radius}, center_{center} {}

  Sphere(const structures::Vec3 &center, std::shared_ptr<Texture_Material> txt,
         std::shared_ptr<Map> m, const double &radius)
      : Object(txt, m), r_{radius}, center_{center} {}

  const double &radius() const { return r_; }

  // const structures::Vec3 at(double i, double j) const override;

  std::optional<intersection_record> intersection(const Ray &r) const override;
  ...
  const structures::Vec3 &center() const { return center_; }
...
public:
  Triangle(std::shared_ptr<Texture_Material> txt, const structures::Vec3 &a,
           const structures::Vec3 &b, const structures::Vec3 &c)
      : Object(txt), vertices_{a, b, c} {
    auto tmp_a = b - a;
    auto tmp_b = c - a;
    normal_ = tmp_a ^ tmp_b;
  ...
  const structures::Vec3 &a() const { return vertices_[0]; }
  const structures::Vec3 &b() const { return vertices_[1]; }
  const structures::Vec3 &c() const { return vertices_[2]; }

  const double &area() const { return area_; }

  const structures::Vec3 &normal() const { return normal_; }
...
public:
  Smooth_Triangle(std::shared_ptr<Texture_Material> txt,
                  const structures::Vec3 &a, const structures::Vec3 &b,
                  const structures::Vec3 &c)
      : Triangle(txt, a, b, c), normals_{normal_ * area_, normal_ * area_,
                                         normal_ * area_} {}
  Smooth_Triangle(std::shared_ptr<Texture_Material> txt, std::shared_ptr<Map> m,
                  const structures::Vec3 &a, const structures::Vec3 &b,
                  const structures::Vec3 &c)
      : Triangle(txt, m, a, b, c), normals_{normal_ * area_, normal_ * area_,
                                            normal_ * area_} {}

  const structures::Vec3 &normal_a() const { return normals_[0]; }
  const structures::Vec3 &normal_b() const { return normals_[1]; }
  const structures::Vec3 &normal_c() const { return normals_[2]; }
...
public:
  Plane(const structures::Vec3 &center, std::shared_ptr<Texture_Material> txt,
        const structures::Vec3 &normal)
      : Object(txt), normal_{normal}, center_{center} {
    structures::unit(normal);
  }
  Plane(const structures::Vec3 &center, std::shared_ptr<Texture_Material> txt,
        std::shared_ptr<Map> m, const structures::Vec3 &normal)
      : Object(txt, m), normal_{normal}, center_{center} {
    structures::unit(normal);
  }
...
public:
  Mesh(smooth_mesh triangles, structures::Vec3 bb_center, double bb_radius)
      : Object(std::make_shared<Uniform_Smooth>(display::Colour(0, 0, 0), 0, 0,
                                                0, 0)),
        triangles_{triangles}, bounding_box_{std::make_shared<Sphere>(
                                   bb_center, txt_, bb_radius)} {}
