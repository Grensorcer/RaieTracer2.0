...
Scene::find_closest_intersection(const Ray &r) const {
  ...
  std::transform(objects_.begin(), objects_.end(),
...
display::Colour Scene::compute_light_input(const Vec3 &intersection_point,
  ...
  for (const auto &light : lights_) {
    ...
    auto light_ray = Ray(intersection_point + light_dir * 0.05, light_dir);
    ...
    if (!light_intersection ||
        structures::norm(light_ray.at(light_intersection->t) -
                         light_ray.origin()) >= light_distance) {
      // Diffusion
  ...
  if (i_r.reflected * i_r.normal > 0)
    res +=
        std::get<0>(i_r.comps) * std::get<4>(i_r.comps) *
        cast_ray(Ray(intersection_point + i_r.reflected * 0.05, i_r.reflected),
                 depth - 1);
  return res;
}
...
display::Colour Scene::cast_ray(const Ray &r, int depth) const {
  ...
  if (depth >= 0) {
    ...
    if (oi_r) {
      ...
      colour = compute_light_input(intersection_point, oi_r.value(), depth);
    } else
...
void Scene::add_object(std::shared_ptr<Object> o) { objects_.emplace_back(o); }

void Scene::add_light(std::shared_ptr<Light> l) { lights_.emplace_back(l); }

void Scene::add_mesh(mesh &m) { add_objects(m.begin(), m.end()); }
