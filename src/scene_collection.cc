...
Scene &scene_one(Scene &s) {
  s.add_light(
      std::make_shared<Point_Light>(structures::Vec3({{-3, -3, 4}}), 3.));
...
Scene &scene_triangles(Scene &s) {
  ...
  auto txt = std::make_shared<Image_Texture>(
      "../data/Paper_Wrinkled_001_basecolor.jpg", 1., 1., 1., 1.);
  auto n_m =
      std::make_shared<Normal_Map>("../data/Paper_Wrinkled_001_normal.jpg");
  for (size_t k = 0; k < nb; ++k)
    for (size_t i = 0; i < nb; ++i)
      for (size_t j = 0; j < nb; ++j)
        s.add_object(std::make_shared<Triangle>(
            txt, n_m, structures::Vec3({{2. - i, -(4. + k), 2. - j}}),
            structures::Vec3({{2. - i, -(4. + k), 3. - j}}),
            structures::Vec3({{3. - i, -(4. + k), 2. - j}})));

  s.add_light(
      std::make_shared<Point_Light>(structures::Vec3({{0, -1, 2}}), 2.));
  s.add_light(
      std::make_shared<Point_Light>(structures::Vec3({{0, -5, 5}}), 5.));

  return s;
}

Scene &scene_smooth_triangles(Scene &s) {
  auto txt = std::make_shared<Uniform_Smooth>(display::Colour(0.5, 0.1, 0.5),
                                              1., 1., 1., 1.);
  // auto txt = std::make_shared<Image_Texture>(
    ...
    for (size_t i = 0; i < nb; ++i)
      for (size_t j = 0; j < nb; ++j)
        s.add_object(std::make_shared<Smooth_Triangle>(
            txt, structures::Vec3({{2. - i, -(4. + k), 2. - j}}),
            structures::Vec3({{2. - i, -(4. + k), 3. - j}}),
            structures::Vec3({{3. - i, -(4. + k), 2. - j}})));
...
Scene &scene_triangle_reflect(Scene &s) {
  s.add_object(std::make_shared<Sphere>(
      structures::Vec3(),
      std::make_shared<Uniform_Smooth>(display::Colour(0.2, 0.3, 0.7), 1., 0.5,
                                       0.5, 1),
      1.));
  s.add_object(std::make_shared<Triangle>(
      std::make_shared<Uniform_Metal>(display::Colour(1., 1., 1.), 0.5, 0.1,
                                      0.1, 1.),
      structures::Vec3({{-1, -2, -0.5}}), structures::Vec3({{1, -2, -0.5}}),
      structures::Vec3({{0, -2, 1.5}})));
  s.add_object(std::make_shared<Triangle>(
      std::make_shared<Uniform_Metal>(display::Colour(0.5, 1., 1.), 0.5, 0.1,
                                      0.1, 1.),
      structures::Vec3({{-2, 0.5, 3.5}}), structures::Vec3({{2, 0.5, 3.5}}),
      structures::Vec3({{0, 2, -1.5}})));

  s.add_light(std::make_shared<Point_Light>(structures::Vec3({{2, 0, 2}}), 2.));
...
Scene &scene_sphere_reflect(Scene &s) {
  s.add_object(std::make_shared<Sphere>(
      ...
      std::make_shared<Uniform_Metal>(display::Colour(0.2, 0.3, 0.7), 1., 0.5,
                                      0.5, 1.),
      1.));
  s.add_object(std::make_shared<Sphere>(
      structures::Vec3({{-1.5, -2, 0}}),
      std::make_shared<Uniform_Metal>(display::Colour(0.9, 0.9, 0.8), 1., 0.5,
                                      0.5, 1.),
      1.));
  s.add_light(
...
Scene &scene_sphere_triangle_reflect(Scene &s) {
  s.add_object(std::make_shared<Sphere>(
      ...
      1.));
  s.add_object(std::make_shared<Triangle>(
      std::make_shared<Uniform_Metal>(display::Colour(0.9, 0.9, 0.8), 0.7, 0.1,
                                      0.1, 1.),
      structures::Vec3({{-1, 2, -2}}), structures::Vec3({{-1, -6, -2}}),
...
Scene &scene_blob(Scene &s) {
  ...
  s.add_object(std::make_shared<Sphere>(
      ...
      std::make_shared<Uniform_Metal>(display::Colour(0.5, 0.5, 0.5), 1., 0.5,
                                      0.5, 1.),
      1.));

  s.add_object(std::make_shared<Plane>(
      structures::Vec3({{0, 0, -2}}),
      std::make_shared<Uniform_Smooth>(display::Colour(0, 0.33, 0.1), 1., 1.,
                                       1., 1.),
      structures::Vec3({{0, 0, 1}})));

  s.add_light(std::make_shared<Point_Light>(structures::Vec3({{2, 0, 3}}), 2));

  return s;
}

Scene &scene_mesh(Scene &s) {
  double reflexion = 0.5;
  auto txt = std::make_shared<Image_Texture>(
      "../data/Seamless_Pebbles_Texture.jpg", 1., 1., 1., reflexion);
  ...
  s.add_object(std::make_shared<Sphere>(
      ...
      std::make_shared<Uniform_Metal>(display::Colour(0.8, 0.2, 0.2), 1., 0.2,
                                      0.2, 1.),
      2.4));
  ...
  s.add_light(
      std::make_shared<Point_Light>(structures::Vec3({{-1, -0.5, 0.5}}), 1));
...
Scene &scene_sphere_texture_reflect(Scene &s) {
  s.add_object(std::make_shared<Sphere>(
      ...
      std::make_shared<Image_Texture>(
          "../data/Paper_Wrinkled_001_basecolor.jpg", 1., 0.7, 0.7, 1),
      std::make_shared<Normal_Map>("../data/Paper_Wrinkled_001_normal.jpg"),
      1.));
  s.add_object(std::make_shared<Sphere>(
      structures::Vec3({0.5, -2, 2.4}),
      std::make_shared<Uniform_Metal>(display::Colour(0.5, 0.5, 0.5), 1., 0.01,
                                      0.01, 1.),
      1.3));
  s.add_light(std::make_shared<Point_Light>(structures::Vec3({0, -1, 1}), 1.));
