...
components Uniform_Texture::get_components(double u, double v) const {
  (void)u;
  (void)v;
...
components Image_Texture::get_components(double u, double v) const {
  size_t i = v * (double)im_.width();
  size_t j = u * (double)im_.height();
  return std::make_tuple<>(im_.get_pixel(j, i), kd_, ks_, ns_, reflexion_);
}
